@php
    ...
    // Editing state (only meaningful inside the Livewire modal). The standalone
    // meeting detail page never sets these, so they default to read-only.
    $isEditing = $editing ?? false;
    $editData = $editableResult ?? $result;
@endphp
    ...
    @else
        ...
        <div class="flex flex-col gap-lg">
            ...
            >
                ...
                <textarea
                    wire:model="editableResult.summary"
                    rows="4"
                    class="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm text-body-md font-sans text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition resize-y"
                >{{ $editData['summary'] ?? '' }}</textarea>

                @error('editableResult.summary')
                    <p class="font-sans text-body-sm text-error italic mt-xs" role="alert">{{ $message }}</p>
                @enderror
            </div>

            <!-- Key Decisions -->
            @if(!empty($editData['decisions']))
            <div
                class="bg-surface-container-lowest border border-outline-variant rounded-lg p-lg"
            >
                <h3
                ...
                <div class="space-y-md">
                    @foreach($editData['decisions'] as $i => $decision)
                    <div>
                        <textarea
                            wire:model="editableResult.decisions.{{ $i }}.text"
                            rows="2"
                            class="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm text-body-md font-sans text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition resize-y"
                        >{{ $decision['text'] ?? '' }}</textarea>

                        @error('editableResult.decisions.' . $i . '.text')
                            <p class="font-sans text-body-sm text-error italic mt-xs" role="alert">{{ $message }}</p>
                        @enderror
                    </div>
                    @endforeach
            ...
            <!-- Action Items -->
            @if(!empty($editData['action_items']))
            <div
                class="bg-surface-container-lowest border border-outline-variant rounded-lg p-lg"
            >
                <h3
                ...
                <div class="space-y-md">
                    @foreach($editData['action_items'] as $i => $item)
                    <div
                        class="flex flex-col gap-xs bg-surface-container-low p-md rounded border border-surface-variant"
                    >
                        <div class="flex flex-col gap-xs">
                            <label class="font-mono text-label-sm text-on-surface-variant">Task</label>
                            <input
                                type="text"
                                wire:model="editableResult.action_items.{{ $i }}.task"
                            ...
                            >
                            @error('editableResult.action_items.' . $i . '.task')
                                <p class="font-sans text-body-sm text-error italic" role="alert">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="flex flex-col gap-xs">
                            <label class="font-mono text-label-sm text-on-surface-variant">Owner</label>
                            <input
                                type="text"
                                wire:model="editableResult.action_items.{{ $i }}.owner"
                                placeholder="Unassigned"
                                class="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm text-body-md font-sans text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition"
                            >
                            @error('editableResult.action_items.' . $i . '.owner')
                                <p class="font-sans text-body-sm text-error italic" role="alert">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="flex flex-col gap-xs">
                            <label class="font-mono text-label-sm text-on-surface-variant">Priority</label>
                            <select
                                wire:model="editableResult.action_items.{{ $i }}.priority"
                                class="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm text-body-md font-sans text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition"
                            >
                            ...
                            @error('editableResult.action_items.' . $i . '.priority')
                                <p class="font-sans text-body-sm text-error italic" role="alert">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="flex flex-col gap-xs">
                            <label class="font-mono text-label-sm text-on-surface-variant">Due Date</label>
                            <input
                                type="date"
                                wire:model="editableResult.action_items.{{ $i }}.due_date"
                                class="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm text-body-md font-sans text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition"
                            >
                            @error('editableResult.action_items.' . $i . '.due_date')
                                <p class="font-sans text-body-sm text-error italic" role="alert">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
            ...
            <!-- Open Questions -->
            @if(!empty($editData['open_questions']))
            <div
                class="bg-surface-container-lowest border border-outline-variant rounded-lg p-lg"
            >
                <h3
                ...
                <div class="space-y-md">
                    @foreach($editData['open_questions'] as $i => $question)
                    <div>
                        <textarea
                            wire:model="editableResult.open_questions.{{ $i }}.text"
                            rows="2"
                            class="w-full bg-surface border border-outline-variant rounded-lg px-md py-sm text-body-md font-sans text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition resize-y"
                        >{{ $question['text'] ?? '' }}</textarea>

                        @error('editableResult.open_questions.' . $i . '.text')
                            <p class="font-sans text-body-sm text-error italic mt-xs" role="alert">{{ $message }}</p>
                        @enderror
                    </div>
                    @endforeach
