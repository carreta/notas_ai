...
2. DUE DATE SOURCE (TD-011): For each action_item, always include "due_date_text" (the raw date expression from the transcript, e.g., "next Friday", "by EOD", "in two weeks"). Set "due_date" to the resolved YYYY-MM-DD date ONLY if a reference date is provided and the expression is resolvable. Set "due_date_source" to "EXPLICIT" if the transcript gives a concrete date, "INFERRED" if you resolved it from a relative expression with a reference date, or "UNKNOWN" if not resolvable or no reference date.
3. MEETING TIME (TD-017): If no meeting_time is provided in the request, treat all due_date fields as UNKNOWN and include the raw text in due_date_text.
